...
public class Tester extends TestCase {
	...
	public void testCallMethod() {
		...
		Integer answer1 = (Integer) ReflectionUtilities.callMethod(mine, "getNumber",
				new Object[] {});
		assertTrue(answer1 == 4);
		String answer2 = (String) ReflectionUtilities.callMethod(mine, "getMessage",
				new Object[] {});
	...
	public void testTypesMatch() {
		...
		Object[][] objectLists = { first, second, third, fourth, fifth, sixth, seventh, eighth,
				ninth };
		...
		Class<?>[][] classLists = { firstClass, secondClass, thirdClass, fourthClass, fifthClass,
				sixthClass, seventhClass, eighthClass, ninthClass };

		for (int k = 0; k < objectLists.length; k++) {
			for (int j = 0; j < classLists.length; j++) {
				if (k == j) {
					System.out.println("True! " + classLists[j]);
					System.out.println("True! " + classLists[k]);
					assertTrue(ReflectionUtilities.typesMatch(classLists[j], objectLists[k]));
				} else {
					System.out.println("FALSE! " + classLists[j]);
					System.out.println("FALSE! " + classLists[k]);
